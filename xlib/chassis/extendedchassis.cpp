...
namespace xlib {
      ...
      * @param irotation senro on the parallel tracking wheel
    ...
    ExtendedChassis::ExtendedChassis (
        ...
        const AbstractMotor::GearsetRatioPair& igearset, const ChassisScales& iscales, const RotationSensor& irotation,
        ...
        odometer.withSensors(irotation, inertial);
    ...
    void ExtendedChassis::driveDistance(QLength target, QTime time) {
        ...
        do {
            double distance = (odometer.getInternalIMU()) - (relativeTrackingDistance).convert(inch);
